...
const ClientLogoItem = ({ src, name }: ClientLogo) => (
    <div className="flex h-16 w-40 shrink-0 items-center justify-center rounded-xl bg-white px-6 py-4 shadow-md opacity-80 transition-all duration-500 hover:opacity-100 hover:scale-105 sm:h-20 sm:w-48 md:h-24 md:w-52">
  ...
  return (
    <div className="relative overflow-hidden border-y border-gray-200 py-10">
      ...
      <div className="space-y-10">
        <div className="marquee-row animate-marquee-left flex w-max items-center gap-6">
          {duplicated.map((client, i) => (
            <ClientLogoItem key={`left-${client.src}-${i}`} {...client} />
          ))}
        </div>
        <div className="marquee-row animate-marquee-right flex w-max items-center gap-6">
          {duplicated.map((client, i) => (
