...
const dbg = (m) => appendFileSync('D:/app-web/.probe-cdp-debug.log', m + '\n');
const ans = (m) => appendFileSync('D:/app-web/.probe-cdp-result.txt', m + '\n');
writeFileSync('D:/app-web/.probe-cdp-debug.log', 'START\n');
writeFileSync('D:/app-web/.probe-cdp-result.txt', 'STARTING\n');
...
async function waitForWs(port, tries = 20) {
...
function cdpEval(wsUrl, expr, ms = 15000) {
  return new Promise((resolve, reject) => {
    ...
    setTimeout(() => { if (!done) sock.write(makeFrame(msg)); }, 600);
  });
}
...
try {
  ...
  dbg('ws=' + ws.slice(0, 90));
  const expr = "JSON.stringify((function(){const it=document.querySelectorAll('.marquee-row > div');return it.length?{vw:Math.round(window.innerWidth),count:it.length,w1:Math.round(it[0].offsetWidth),h1:Math.round(it[0].offsetHeight),cls1:it[0].getAttribute('class')}:{err:'no items'};})())";
